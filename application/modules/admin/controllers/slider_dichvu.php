<?php
class slider_dichvu extends MY_Controller{

	public function __construct(){
		parent::__construct();
        $this->_data['now'] = 'Slider Dịch Vụ';
        $this->load->model('mslider_dichvu');
        ...
        $this->_gallery_url = base_url()."public/slider_dichvu/";
        //Lấy đường dẫn vật lý của thư mục chứa hình ảnh đươc upload
        $this->_gallery_path = realpath(APPPATH. "../public/slider_dichvu");
	}
	public function listall(){

		$this->_data['template'] = 'admin/slider_dichvu/list_view';
		$this->_data['title'] = 'Trang Quản Lý Danh Mục ';
		$this->_data['info'] = $this->mslider_dichvu->listAll();
		...
		if(isset($_POST['ok'])){
            ...
            if($this->form_validation->run()==TRUE){
                ...
                if(!$this->upload->do_upload("image")){
                    ...
                    $this->mslider_dichvu->add($arr);
                    $this->session->set_flashdata('ses_flash',"Thêm");
                    redirect(base_url()."admin/slider_dichvu/listall");
                }

			}
		}
		$this->_data['template'] = 'admin/slider_dichvu/modify_view';
	...
	public function del($id){
        $slider_dichvu = $this->mslider_dichvu->getById($id);
        unlink($this->_gallery_path.'/'.$slider_dichvu['image']);
		$this->mslider_dichvu->del($id);
	...
	public function edit($id){
		...
		if(isset($_POST['ok'])){
            ...
            if($this->form_validation->run()==TRUE){
                if($_FILES['image']['name']==""){
                    ...
                    $this->mslider_dichvu->editById($id,$arr);
		...
		$this->_data['info'] = $this->mslider_dichvu->getById($id);
		$this->_data['template'] = 'admin/slider_dichvu/modify_view';
