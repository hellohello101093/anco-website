<?php
class slider_lienhe extends MY_Controller{

	public function __construct(){
		parent::__construct();
        $this->_data['now'] = 'Slider Liên Hệ';
        $this->load->model('mslider_lienhe');
        ...
        $this->_gallery_url = base_url()."public/slider_lienhe/";
        //Lấy đường dẫn vật lý của thư mục chứa hình ảnh đươc upload
        $this->_gallery_path = realpath(APPPATH. "../public/slider_lienhe");
	}
	public function listall(){

		$this->_data['template'] = 'admin/slider_lienhe/list_view';
		$this->_data['title'] = 'Trang Quản Lý Danh Mục ';
		$this->_data['info'] = $this->mslider_lienhe->listAll();
		...
		if(isset($_POST['ok'])){
            ...
            if($this->form_validation->run()==TRUE){
                ...
                if(!$this->upload->do_upload("image")){
                    ...
                    $this->mslider_lienhe->add($arr);
                    $this->session->set_flashdata('ses_flash',"Thêm");
                    redirect(base_url()."admin/slider_lienhe/listall");
                }

			}
		}
		$this->_data['template'] = 'admin/slider_lienhe/modify_view';
	...
	public function del($id){
        $slider_lienhe = $this->mslider_lienhe->getById($id);
        unlink($this->_gallery_path.'/'.$slider_lienhe['image']);
		$this->mslider_lienhe->del($id);
	...
	public function edit($id){
		...
		if(isset($_POST['ok'])){
            ...
            if($this->form_validation->run()==TRUE){
                if($_FILES['image']['name']==""){
                    ...
                    $this->mslider_lienhe->editById($id,$arr);
		...
		$this->_data['info'] = $this->mslider_lienhe->getById($id);
		$this->_data['template'] = 'admin/slider_lienhe/modify_view';
